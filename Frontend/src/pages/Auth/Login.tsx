...
const loginSchema = yup.object().shape({
  userOrEmail: yup.string().required('Required!!').max(80),
  password: yup.string().max(255).required('Password is required'),
  rememberMe: yup.boolean().oneOf([true, false], 'Must be true or false'),
});
...
const Login: FC<Props> = () => {
  ...
  const onSubmit = (form: ILoginSchema) => {
    loginUser(form.userOrEmail, form.password, form.rememberMe ?? false);
  };
          ...
          >
            <div className='flex w-full flex-col items-start justify-start gap-2'>
              <label className='text-xs text-slate-200'>
                Username or email{' '}
                {errors?.userOrEmail && (
                  <p className='text-red-500 text-xs inline-block ms-2'>{errors.userOrEmail.message || 'Required!!'}</p>
                )}
              </label>
              <input
                ...
                type='text'
                {...register('userOrEmail', { required: true, maxLength: 80 })}
              />
            </div>
            <div className='flex w-full flex-col items-start justify-start gap-2'>
              <label className='text-xs text-slate-200'>
            ...
            <div className='mr-4 flex items-center'>
              ...
              <div className='ml-3 text-sm leading-6'>
                <label
                  htmlFor='checkbox-2'
                  className={`text-sm font-medium text-white ${errors?.rememberMe && 'text-rose-500'}`}
                >
                  Remember me
                </label>
              </div>
            ...
            <p className='text-sm font-light text-white'>
              Don't have an account? <span className='cursor-pointer font-bold hover:underline'>Create an account</span>
            </p>
          </form>
        </div>
      </div>
